...
function Climbable:collide( node, dt, mtv_x, mtv_y )
    ...
    if not player.isClimbing and not player.key_down['JUMP'] then
        if ( player.key_down['UP'] and player_base > self.position.y + 10 ) or
           ( player.key_down['UP'] and node.velocity.y ~= 0 ) or
           ( player.key_down['DOWN'] and player_base < self_base - 10 and player_base > self.position.y + 10 ) then
            self:grab( player )
        end
    end
    ...
    if player.isClimbing and ( player.jumping or
        ...
        ( p_width >= self.width and player.key_down['LEFT'] ) or 
        ( p_width >= self.width and player.key_down['RIGHT'] ) or
